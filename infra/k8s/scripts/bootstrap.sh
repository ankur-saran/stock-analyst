...
echo "  vault operator unseal   # x3, using the unseal keys from init"
echo "  ./scripts/configure_vault.sh"
echo "configure_vault.sh must create the Kubernetes-auth roles referenced by"
echo "each SecretProviderClass's roleName (postgres, minio, app, litellm) and"
echo "write the KV secrets at the paths in"
echo "infra/helm/stock-analyst-infra/values.yaml's vaultSecrets list, plus a"
echo "'vault-auth' ServiceAccount in stock-analyst-data/stock-analyst/stock-analyst-infra"
echo "bound to those roles."
